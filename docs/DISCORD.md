...
- `/studio preview <track>` — render and attach a bounded, at-most-30-second Dummy MP4 centered around the selected track using the complete current mix. Because the window follows the track after a move, the job embed shows the track's absolute offset and preview window range. It runs in the preview worker pool.
