...
- `/touchflavor <text>`, `/lsflavor [page]`, `/rmflavor <index>` — **upper-tier**; manage global idle presence flavor lines in `DB/config/global/environment/flavor.pandora`. When the queue is empty, presence randomly picks one stored line instead of `No jobs in queue.`; if none exist, the default text remains.
