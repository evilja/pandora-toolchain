...
Use `/providers` to verify that the Worker reports the global/guild Drive profile and each configured streaming host. A server configured with `/edit drive_only:true` reports the external streaming hosts as disabled by server policy even when their Worker secrets are present; use `/edit drive_only:false` to restore them for future releases. `/edit hls:true` makes Lumiere Files HLS the exclusive release output for that server: no Drive or external-host upload starts; release encodes write that layout themselves — the job's last pnmpeg run, whether that is the AOT final mux, the preset encode, or the intro concat, emits it into the job's `work/hls`, which the upload worker adopts with one rename — and anything that still produced an MP4 (a VerySlow parallel encode, a failed direct mux) has it stream-copied into a master playlist (`<name>.m3u8`) and media playlist (`<name>_variant.m3u8`). The source codec selects the segment layout: H.264 uses MPEG-TS chunks (`chunk/p<n>-<name>.ts`), while AV1 uses fMP4/CMAF (`chunk/init-<name>.mp4` plus `p<n>-<name>.m4s`). `<name>` comes from that server's `/edit hls_name:` template — `%uuid%`, `%random%`, and `%res%`, defaulting to `%uuid%_%random%_%res%` — rendered for the output being published; an adopted layout keeps the name the encoder already gave it. The media validator requires the exact init/fragment names belonging to that layout, an fMP4 layout is published only when its init segment really carries AV1, the master advertises `RESOLUTION` and whole RFC 6381 `CODECS` values (`avc1.640028`, `av01.0.08M.10`) — read from ffprobe's `mime_codec_string`, or assembled out of the profile and level for H.264/AAC when the ffprobe in `DB/bin` is too old to have that field (the bundled portable build is FFmpeg 7.0.2), and left off entirely when neither route can name a stream, since a bare codec family such as `avc1` does not parse and makes a player drop the video track and play the release as audio over a black frame — and the route serves playlists, TS, CMAF fragments, and init segments with distinct content types. The random capability is retained on the VDS for 12 hours and its master URL is the job's sole advertised link. `/edit` accepts AV1 when HLS-only or Drive-only is enabled; the capability and expiry survive `pndc` restarts, and expired outputs are removed by the worker cleanup loop and opportunistically on access/publication.
