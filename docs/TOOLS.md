...
- `--drive --env env.pandora`: uploads `--link` (local file) to Drive + Doodstream + Lulu + Voe + Abyss in parallel, streaming initialization as protocol opcode `4`, progress as opcode `0`, and results as opcode `1`/`2` per host. Initialization payloads are `["4", host_id, message]` and cover OAuth, Drive-folder resolution, upload-server discovery, and connection setup. Upload sends are unlimited/no request deadline (clients only set a 60s connect timeout); upload streams use 256 KiB read buffers and known content lengths for every provider, including Drive. Source progress events are throttled to 500ms to avoid per-chunk channel traffic; the protocol payload emits the total file size once plus `[done, extension_count]` per host, currently with extension count normally `0`, rendered as e.g. `Doodstream 11/1032 MB`. The first protocol progress payload is emitted immediately; later protocol progress payloads are throttled to roughly 5s from the previous emitted progress. Resolved Drive paths are persisted under `DB/cache/drive-folders` by root parent + normalized path so later `pncurl` processes skip repeated folder search/create calls.
