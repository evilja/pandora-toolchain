...
- `admin.pandora` — `/hearts`, `/gitsync`, `/configure`, `/edit`, `/touchapi`, `/gettranslation`, `/touchtranslation`, `/gettranslationall`, `/touchtranslationall`, `!auth`, `!ban`
...
- `/akiraconfirm <job_id> <episode> <name> [slug] [folder]` — **rank 4 (Witch tier)**; creates or refreshes the Akira episode for the given anime slug and episode number, then replaces its episode links with the job's uploaded links. If `slug` is omitted, it uses the channel's attached anime slug from `/attach` or `/init`. The Drive upload is published as an Akira index player URL under `folder` (defaults to the anime slug), using `name` as the index filename.
