...
pub fn build_spec(
    ...
    LinkJobSpec {
        ...
        logo_b64: job.server_logo.as_ref().map(|logo| encode_base64(&logo.bytes)),
        logo_extension: job.server_logo.as_ref().map(|logo| logo.extension.clone()),
        logo_placement: job.server_logo.as_ref().map(|logo| logo.placement.clone()),
