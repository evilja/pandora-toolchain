...
- `/studio disown` / `/studio reown [studio_id]` — leave the current Studio or join a previous/shared Studio. IDs can be shared with authorized users in the same guild for concurrent collaboration. A user may own multiple Studios but has one current selection; active Studios expire after 24 hours without a successful Studio command, and Studios with no collaborators expire after 30 minutes. The HTTP Studio API mirrors these operations for local tokens.
