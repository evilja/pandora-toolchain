...
- `POST /api/v1/subs/media` — a video for the browser subtitle editor (see [SUBTITLES.md](SUBTITLES.md#video-from-a-link))
...
- `GET /subs` → Pandora Subs, the client-only subtitle editor (`web/subs.html`), with `GET /subs/{ass,render,app}.js` and `GET /subs/manifest.webmanifest`; it works without the API, and calls it only to fetch a video from a link — see [SUBTITLES.md](SUBTITLES.md)
