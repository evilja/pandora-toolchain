...
- `POST /api/v1/link/register`, `GET /api/v1/link/lease`, `POST /api/v1/link/lease/:id/renew`, `POST /api/v1/link/lease/:id/result`, `GET /api/v1/link/assets/manifest`, `GET /api/v1/link/assets/:hash` (link token only — see [LINK.md](LINK.md)). The asset routes serve the font/intro corpus a node syncs; `:hash` is a content hash and is served only when the current manifest lists it, so a node can never address a path on the coordinator's disk.
