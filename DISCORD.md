...
- `/akiraconfirm <job_id> <episode> <name> [slug] [folder]` — **rank 4 (Witch tier)**; creates the Akira episode if missing, or updates only the existing episode's link fields if it already exists, then replaces its episode links with the job's uploaded links. If `slug` is omitted, it uses the channel's attached anime slug from `/attach` or `/init`. The Drive upload is published as an Akira index player URL under `folder` (defaults to the anime slug), using `name` as the index filename.
