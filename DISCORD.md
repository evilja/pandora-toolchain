...
- `/akiraconfirm <job_id> <slug> <episode> <name> [folder]` — **rank 4 (Witch tier)**; creates or refreshes the Akira episode for the given anime slug and episode number, then replaces its episode links with the job's uploaded links. The Drive upload is published as an Akira index player URL under `folder` (defaults to the anime slug), using `name` as the index filename.
