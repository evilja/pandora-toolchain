...
`JobType::SubsMedia` is the browser editor's variant (`POST /api/v1/subs/media`, see [SUBTITLES.md](SUBTITLES.md#video-from-a-link)): it queues through `queue_subs_job` and is dispatched to the preview pool the same way, as `WorkerMsg::SubsMedia`, where `subs_media::run_subs_media_job` probes the input, extracts its text tracks with the same `extract_subtitle_tracks` helper `/subs` uses, and runs one ffmpeg pass that writes a 540p H.264/AAC `video.mp4` and streams mono 8 kHz PCM to stdout, folded into 100-per-second peaks (`peaks.bin`, one byte each). The result lands in `DB/cache/subsmedia/<token>/` with a `manifest.json` and a `.expires` 12 hours out, staged as `.<token>.tmp` and renamed into place; `SUBSMEDIA_DONE` carries the token (never printed in the message text) at `Stage::Uploaded`, and `persist_side_effects` writes it to `progress` as `{type:"subsmedia", ready, token}`. Proxy progress is `{type:"subsmedia", percent}` from ffmpeg's `-progress` output; a failure is `{type:"subsmedia", error}`. Expired and stale staging directories are swept at startup and each minute alongside HLS. It is refused on an orchestrator and is not leasable, because the files it makes are served from this machine.
...
- Every encode input is copied into `DB/cache/inputs/<key>/input.mkv` (once; a source with a second cache key gets a hard link to that copy, falling back to a copy where linking fails) when a job reaches `Encoded`, and every freshly downloaded preview input is cached when it reaches `Downloaded`; a cancelled job also caches if it was already past `Downloaded` (`Downloaded`, `Encoding`, `Encoded`, `Uploading`, `Uploaded`). Preview and encode jobs can therefore reuse the same source within the cache TTL. New uses reset the cache timer to 30 minutes (`INPUT_CACHE_TTL_SECS`). `pn_worker` runs `cleanup_torrent_runtime()` at startup to clear stale cross-process torrent locks, while valid input-cache entries survive; startup and the background cleanup tick evict only cache dirs whose `touch` file is older than the TTL. The input-cache key (`input_cache_key`) is `md5(torrent.get() | probe_file_index)`; `use_cache_or_wait` (run at download dispatch) first tries a cache copy, then falls back to waiting on an in-queue duplicate (`queued_duplicate_source`).
- A probe (the listing a pack-taking command runs, or the API's `POST /jobs/probe`; there is no Discord `/probe`) does **not** support GDrive or direct video links — `pn_probeworker` fails the job immediately, and `Job::pick_file_first` never asks for one.
- **Probe row order and paging**: `format_probe_rows` (probeworker) detects an episode number per file — first the direct `- 12` / `S01E12` / `E12` regexes, else `sequence_tokens`, which picks the numeric column that counts up across the file list — and, when at least two files match, sorts the rows by that number (`12v2` sorts after `12`; files with no number keep torrent order at the bottom). The displayed `` `n` `` stays the torrent's file index, since that is what a pick typed in chat, a batch's index selection, and the API's `file_index` select. `pnworker/probe_pages.rs` then chunks the rendered list into 10-line / 900-char pages (embed field values cap at 1024) and builds the `pnprobe:<job_id>:<page>` buttons; `Frontend::update` attaches them for `PROBE_ROW` and sends an empty component list for every other payload so stale buttons cannot survive on the message. `handlers/probe.rs::handle_probe_component` serves a page click by re-reading the full list from the job's `progress` JSON and rewriting the clicked embed, so paging survives a `pndc` restart and keeps no in-memory state. The rendered rows are `progress.file_text`; `files` beside them is the structured array the console reads, and only held the rendered text itself on rows written before the two were split — reading `files` as a string is what made every page button answer `PROBE_PAGE_EXPIRED`.
- <a id="source-names"></a>**Source names**: the jobs table's `source_name` is the name the encoded file had inside its torrent. It is written down because it cannot be read back later — the downloader renames the file to `input.mkv` and the work directory is cleared when the job ends, so afterwards only a fresh fetch of the torrent would say. Two places know it and both record it (`JobDb::set_source_name`): `promote_picked_job`, from the listing's `files[].name` for the chosen index (or the only file, when nothing had to be chosen) — the one moment a coordinator that leases its downloads ever sees the name — and `downloadworker::record_source_name`, just before the rename, which also covers jobs that never list (a `Pancode` from a recorded `SOURCE.md` index, API jobs). Only the last path component is kept. A job served from the duplicate cache with no listing, a Drive or direct link, and a leased job that never listed have none; the Discord [`job` option](DISCORD.md#the-job-option), which is what reads it, falls back to the link.
- <a id="picking-a-file-out-of-a-pack"></a>**Picking a file out of a pack**: a Discord `/encode do|link|keep`, `/smartcode do|keep`, `/subs` or `/backup` whose source is a torrent or magnet calls `Job::pick_file_first` (it accepts `Encode`, `Subs` and `Backup` — every job that works on exactly one video), which stores the job's real type in `Job.pick_then` and turns the job into a `JobType::Probe` for its first phase. Being a probe to everything that moves it is the point: the queue, the preview pool, and a node's lease on an orchestrator all run the listing through the path a probe has always had, with nothing taught about picks. `create_job_embed` titles the message by `pick_then`, so it reads as the encode it is. When the listing lands at `Probed`, `do_pick_things` reads the offered indices back from the job's stored `progress` (`files[].index` — the one place both a local probe and a node-answered one leave them). **One video**: the job is promoted at once with no file index, so it downloads exactly what it did before listing existed. **More than one**: `Job.pick_files` records the indices, and the `PROBE_ROW` render carries `PICK_PROMPT` as the embed description above the usual paged list (the page buttons keep the description when they rebuild the embed). pndc's `message` handler forwards every bare-number message from a non-bot as `JobClass::Pick(PickRequest { author, channel_id, index })` without an authorization check, since a number can only select for a job its author already queued; `pick_target` matches it against the newest job that is at `Probed`, has `pick_then`, belongs to that author and channel, and offered that index — everything else is dropped silently. The request carries the chat message as a `Frontend`, and a number that did select is deleted (off the queue loop) so the channel keeps only the job message; this needs Manage Messages, and where the bot lacks it the delete fails quietly and the number stays. A number that matched nothing is never touched. `promote_picked_job` restores `job_type`, sets `probe_file_index` and a `<source> • file #<n>` display link, resets the stage to `Queued`, and runs `admit_job` — the second half of the intake, split out so a promoted job takes the same node-or-local route a new one does without re-answering the queue-length and orchestrator refusals. The job keeps its id, message, and work directory, so a `.torrent` the listing fetched is the one the download selects against; `insert_job` is an upsert and rewrites the row's `job_type`. `queue_encode_job` passes `probe_file_index` to the downloader, and cache and forward keys already include it. A `keep` request is set aside in `Job.pick_keep` for the listing phase and restored on promotion, because admitting a keep reserves its keyword and the encode would otherwise be refused the keyword its own listing took. A pick job unanswered for `PROBE_SELECTION_TIMEOUT` (180s) is cancelled with `PICK_TIMEOUT` instead of having its message deleted the way a bare probe's is. While listing, the job still counts as an encode for the `/gitsync` gate. Web/API submits never pick (there is no chat to answer in) and keep the unselected download; the API's `pancode` route is unchanged. The worker records a chat-picked index nowhere itself, but a job may carry a `pick_answer` sender, and `promote_picked_job` sends the index down it when a person chose one; `/smartcode` uses that to write the pick into the episode's `SOURCE.md` so a re-encode does not ask again. `/source` pins an episode to a file the same way and asks the question from its own handler because it queues no job — see [DISCORD.md](DISCORD.md). Two commands need the list before they have anything to queue: `/source`, and `/encode do` with a subtitle archive, which pairs the list against the archive to build a batch. Both call `handlers/listing.rs::list_source`, which submits a plain `JobType::Probe` through `Job::new_api` (so it has a `Web` frontend and touches no message), polls its row until `Probed`, and reads the rows back with `probe_rows`. That probe sits out its ordinary 180s window and is what the resulting batch or `SOURCE.md` names as its `probe_job_id`.
