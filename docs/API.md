...
`Dockerfile` (multi-stage — builds all workspace bins, runtime image bundles `ffmpeg`) + `docker-compose.yml` run `pndc` alongside a `cloudflared` sidecar on a shared network with **no published ports**; the Cloudflare tunnel's public-hostname service points at `http://pndc:8787` (the compose service name, not `localhost`). The build requires `PNX264_RELEASE_URL` and `PNX264_RELEASE_SHA256`, pinning the x264 fork's Linux x86_64 static release bundle (`include/x264*.h` + `lib/libx264.a`) before Rust compilation; an unset URL or mismatched digest fails the image build rather than silently linking distro x264 without plan-only support. `DB/` is bind-mounted so the database, env, and tokens persist. Lumiere remote uploads may use a second public Tunnel hostname pointing at the same service; that hostname must allow provider access to `/lumiere/v1/files/*`. See `web/README.md` and [LUMIERE_BROKER.md](LUMIERE_BROKER.md).
